...
Будем счиать что один сервер базы данных имеет 2 Tib RAM

Тогда на ЦОД нужно  ``` 93.7 / 2 = 47 ``` шардов

На одноном шарде будет хранится ``` 2 TiB / 0.771 Mib/10k = 27.1 bln ``` записей

Которые будут весить(посчитаем по файлам) ``` 27.1 bln * 446 B = 11 TiB ```

**Количество реплик**

Нагрузка на один шард (будем считать как нагрузка * 18% / 47 )

| Запрос   | RPS | ТИП|
|---|---|--|
| Загрузка на диск | 1_100 |запись|
| Скачивание с диска | 2_300 |чтение|
| Просмотр директории | 20 |чтение|
| Поиск по диску | 20 |чтение|

Сделаем какдому шарду по 3 реплики и 1 скрытую для надежности, так как реплики должны деждать нагрузку на запись тоже

Итого размер на один ЦОД на FileDB выходит(учитывая реплики) ``` 3_102TiB ```
...
Просмотр по ссылке посчитам как ```% файлом со ссылкой * rps просмотра```
...
По характеристикам видно что эти базы спокойно помещаются в один сервер
