...
Допустим пользователь в среднем зологинен с двух устройств
...
Допустим только 5% файлов имеют расширенные права доступа и в среднем на 2 человек
  ...
  * file_uid - файл к которому он имеет права ( 16 B )
  * permisson - тип прав (чтение, редактировани) ( 1 B )

``` 33 B * 3200 * 0.05 * 2 * 2.6 bil = 25 TiB ```

## Физическая схема данных
### PostgreSql

В postgreSQL будем хранить информацию связанныю с файлами, а именно таблицу файлов и таблицу версий файлов.

### Tarantool

Для таблиц пользователей, ACL и версий будем использовать Tarantool с фреймворком Cartridge
