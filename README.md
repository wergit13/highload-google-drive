...
Большая часть аудитории находится в США (25 %), другие же основные места это северная часть Южной Америки(Мексика, Бразилия) и Азия(Индия, Япония), остальной трафик равномерно приходит с Европы. 
...
Возможен вариант использования Geo Based Dns балансировки (например Amazon Route 53, так как они имеют достаточно большую карту маппинга ip и геопозиции).

Так как показатель Latency при большой нагрузке на траффик файлового хранилища требует минимизации, в пределах США и Европы будем использовать BGP Anycast для выбора нужного ЦОД'а.
В США будет использоваться один IP для AS, который будет сопостовлять с IP настоящего ЦОД'а.
Разобьем все линки BGP сети условно на 4 группы, каждая из которых будет соответствовать одному из четырех ЦОД'ов.
...
## Локальная баланчировка

Балансировка будет осуществляться посредством роутинга и L7 балансировщиков.

После приземления клиента в цод маршрут выбирается на основании **BGP Routing'а** поскольку у диска можеть быть большой входящий трафик стоит использоть сетевое оборудование. Будем использовать **Per-Flow** алгоритм хэширования, чтобы пакеты с одного IP попадали на один и тот же балансировщик. Роутер будет связан с репликасетом с одним вирутальным IP, резервируемым с помощью **keepalived VRRP** .

В качестве L7 балансировщика будем использовать **nginx**, он же будет и отвечать за SSL терминация. **Nginx** будет находится под kubernetes который будет осущесвлять liveness пробы. И обновлять конфигуцию балансировщика. Балансировщики так же будут стоять с паре через keepalived для отказоустойчивойсти.
